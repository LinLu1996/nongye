                ...
                return <div>
                    {
                        ...
                        (securityKeyWord.indexOf('platform_all') !== -1 || record.roleType === 3) &&
                        <a onClick={this.query.bind(this,record)}>编辑</a>
                    }
                    {
                        (Com.hasRole(securityKeyWord, 'role_delete', 'delete', 'role')) &&
                        ((record.roleCode !== 'system2company' && record.roleCode !== 'system2user') && (securityKeyWord.indexOf('platform_all') !== -1 || record.roleType === 3)) &&
                        <span>
                            {
                                (Com.hasRole(securityKeyWord, 'role_update', 'update', 'role')) &&
                                (securityKeyWord.indexOf('platform_all') !== -1 || record.roleType === 3) &&
                            ...
                            <span>
                                {
                                    ...
                                    (securityKeyWord.indexOf('platform_all') !== -1 || record.roleType === 3)) || (
                                        (Com.hasRole(securityKeyWord, 'role_delete', 'delete', 'role')) &&
                                        ((record.roleCode !== 'system2company' && record.roleCode !== 'system2user'))
                                    )) &&
                                    <Divider type="vertical"/>
                                }
...
delete(record) {
    ...
    IO.role.roleDelete({':id': ids}).then((res) => {
        if (res.success) {
  ...
  onTableChange(pagination, filters, sorter) {
    ...
    this.props.getpsize(pagination.pageSize);
  ...
  render() {
    ...
    return (
            <div className='res-table'>
                 <LocaleProvider locale={zhCN}><Table columns={arr}  pagination={paginationOpt} onChange={this.onTableChange.bind(this)} dataSource={data} loading={flag}/></LocaleProvider>
            </div>
    );
  }
}

...
class modifyModal extends Component {
    ...
    DataFn(type) {
        ...
        if (type === 'add') {
            IO.employee.employeeAdd(obj).then((res) => {
                if(res.success) {
                    this.requestFn('employee_add', '添加成功');
                }
            }).catch((res) => {
                Com.errorCatch(res);
                this.setState({
            ...
            if (contrast.indexOf(false) !== -1) {
                IO.employee.employeeUpdate(obj).then((res) => {
                    if(res.success) {
                        this.requestFn('employee_update', '修改成功');
                    }
                }).catch((res) => {
                    Com.errorCatch(res);
                    this.setState({

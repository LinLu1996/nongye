                        ...
                        type === 'detail' ? moment(palnInfo.plannedTime).format('YYYY-MM-DD') :
                            getFieldDecorator('plannedTime', {
                                initialValue: moment(palnInfo.plannedTime)
                                    ...
                                    <DatePicker defaultValue={moment(palnInfo.plannedTime)} onChange={(e) => {
                                        this.props.setPlanTime(e);
                                    }}/>
                                </LocaleProvider>
                            )
                    }
                ...
                <FormItem label="计划用量" {...formItemLayout}>
                    {
                        (type === 'detail' ? ((palnInfo.materialId && palnInfo.materialId !== -1) ? palnInfo.plannedQty : '无') :
                            ((type !== 'create' && (!palnInfo.materialId || palnInfo.materialId === -1)) ? '无' : getFieldDecorator('plannedQty', {
                                ...
                                <div><InputNumber min={0} disabled={!this.state.materialFlag}
                                                  defaultValue={palnInfo.plannedQty} onBlur={(e) => {
                                    this.props.setPlanQTY(e);
                                }}/>
            ...
            if (this.state.planStatus && this.state.planStatus === 'pending') {
                        ...
                        return (
                            <div>
                                {record.deleted === 0 ? <div>
                                        ...
                                        <Divider type="vertical"/>
                                        <a onClick={(item) => {this.hanleOperate('del', item, record);}}>删除任务</a>
                                        {record.isRequest === 0 ?
                ...
                farmPlanColumns.push({
                    ...
                    render: (text, record) => {
                        return (
                            <div>
                                {record.deleted === 0 ? <a onClick={(item) => {
                                            this.hanleOperate('pic', item, record);
                                        }}>现场照片</a> :
                                    <a>
                                        <a>现场照片</a>
                                ...
                                {record.deleted === 0 ? <div>
                                       <a onClick={(item) => {
                                        ...
                                        {record.isRequest === 0 ?
                                            <span onClick={(item) => {
                                                this.hanleOperate('req', item, record);
                                            }}><Divider type="vertical" /><a>发起用工请求</a></span> :
                                            <span><Divider type="vertical" /><a>已经发过用工请求</a></span>}
                                    </div> :
                                    <div>
                                        <a>任务调整</a>
                                        <Divider type="vertical" />
                ...
                <div className='content'>
                            ...
                            <section>
                                ...
                                {this.state.pageType === 'modify' ? <ul className='planDetail'>
                                        ...
                                        {planInfo.workplanStatusCode === 'pending' ? <li className='plant-date-li'>定植日期：
                                                <LocaleProvider locale={zhCN}>
                                                    <DatePicker value={this.state.confirmDate}
                                                                onChange={this.confirmTime.bind(this)}/>
                                                </LocaleProvider>
                                                <Button className='content-button' type='primary'
                                                        onClick={this.handlePlanDate.bind(this)}>确认定植日期</Button>
                                            </li> :
                                            <li>定植日期：{planInfo.plantingDate !== null ? moment(planInfo.plantingDate).format('YYYY-MM-DD') : ''}</li>}
                                    </ul> :
                                    <ul className='planDetail'>
                                        <li>计划开始日期：{planInfo.startTimeFormat}</li>
                                        {planInfo.workplanStatusCode === 'pending' ?
                                            <li className='plant-date-li'>定植日期：{moment(planInfo.confirmDate).format('YYYY-MM-DD')}
                                            </li> :
                                            <li>定植日期：{planInfo.plantingDate !== null ? moment(planInfo.plantingDate).format('YYYY-MM-DD') : ''}</li>}
                                    </ul>}
                                {this.state.pageType === 'modify' ? <ul className='planDetail'>
                                    <li className='content-button-list plan-detail-button'>
                                        {planInfo.workplanStatusCode === 'pending' && <Button type='primary'
                                                                                              onClick={this.handlePlanDoneConfirm.bind(this)}>计划完成</Button>}
                                        {planInfo.workplanStatusCode === 'pending' && <Button type='primary'
                                                                                              onClick={this.handlePlanCancelConfirm.bind(this)}>终止计划</Button>}
                                    </li>
                                </ul> : <ul></ul>}
                            </section>
                        </ul>
                    ...
                    <div className="content-wrap">
                        <div className='table-header'>
                            ...
                            <p><Button type="primary" onClick={this.handleViewRecord.bind(this)}>查看修改记录</Button></p>
                        </div>
                        <div className='res-table scroll-table'>
                            <LocaleProvider locale={zhCN}>
                                <Table rowKey={record => record.id}
                                       rowClassName={this.setDeletedRow.bind(this)} columns={farmPlanColumns}
                    ...
                    <div className="content-wrap">
                        {this.state.pageType === 'modify' ? <div className='table-header'>
                                ...
                                {planInfo.workplanStatusCode === 'pending' &&
                                <p><Button type="primary" onClick={this.hanleOperateCreate.bind(this)}>添加临时任务</Button></p>}
                            </div> :
                            <div className='table-header'>
                                <p><i className='iconfont icon-sort'></i><span>临时任务列表</span></p>
                            </div>}
                        <div className='res-table scroll-table'>
                            <LocaleProvider locale={zhCN}>
                                <Table rowKey={record => record.id}
                                       rowClassName={this.setDeletedRow.bind(this)} columns={temporaryColumns}
                                       dataSource={temporaryTaskList} pagination={false}
                                />
                            </LocaleProvider>
                        </div>
                    </div>
                    ...
                    <div className="content-wrap">
                        ...
                        <div className='res-table scroll-table'>
                            <LocaleProvider locale={zhCN}>
                                <Table rowKey={record => record.id} columns={reListColumns}
                                       dataSource={receiveList} pagination={false}/>
                            </LocaleProvider>
                        </div>
                    </div>
                    ...
                    <Modal title={this.state.title}
                        ...
                        <Form layout="inline" className='ant-form'>
                            <FormItem label="预计用工人数">
                                <div><InputNumber min={0} value={this.state.personQty}
                                                  onChange={this.handlePersonQty.bind(this)}/>
                                </div>
                            </FormItem>
                        </Form>
                    </Modal>
                    ...
                    <Modal
                        ...
                        width={550}>
                        <TaskFromNew setPlanQTY={this.setPlanQTY.bind(this)} setPlanTime={this.setPlanTime.bind(this)}
                                     type={this.state.type} palnInfo={this.state.taskInfo}
                                     workTypeList={this.props.workTypeList} material={this.material.bind(this)}
                                     operation={this.operation.bind(this)} user={this.user.bind(this)}
                                     userList={this.state.userList} landList={this.detailData.astLands}
                    ...
                    <Modal title={this.state.title}
                           ...
                           width={1200}>
                        <LocaleProvider locale={zhCN}>
                            <Table rowKey={record => record.id} dataSource={this.state.editRecordList}

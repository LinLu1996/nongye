...
class modifyModal extends Component {
    ...
    hideModal() {   //点击确定的回调
        const {name, longitude, latitude, phone, address, user,companyId,area, stauts} = this.props.fields;
        const {modaltype,updateTree,Cur,Psize,userId,names,baseId} = this.props;
        //e.preventDefault();
                      ...
                      if (res.success) {
                        if (res.data > 0) {
                          message.success('添加成功');
                            this.props.Alldatas({startPage: 1, limit: Psize,name:names,userId:userId,baseId});
                    ...
                    IOModel.Modifydata(modifydata).then((res) => {  //修改成功时的回调
                      if (res.success) {
                        if (res.data > 0) {
                          ...
                          this.props.Alldatas({startPage: Cur, limit: Psize,name:names,userId:userId,baseId:this.props.selectID});
                          updateTree();
                        } else {

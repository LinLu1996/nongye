...
const CustomizedForm = Form.create({
    ...
    const {modeltype_land,EditData, Rditdate} = props;
    let isEdit = false;
    let latitude;
    let longitude;
    let GIS;
    JSON.stringify(EditData)!=='{}' ? latitude=EditData.latitude : latitude=Rditdate.latitude;
    JSON.stringify(EditData)!=='{}' ? longitude=EditData.longitude : longitude=Rditdate.longitude;
    latitude && longitude ? GIS = `http://geojson.io/#map=10/${latitude}/${longitude}` : GIS = `http://geojson.io/#map=10`;
    //let createTime = "";
    ...
    return (
        <Form layout="inline">
            ...
            <div className='tishi'>{getFieldDecorator('gisData', {
                ...
                <Tooltip autoAdjustOverflow={false} overlayClassName='toolp' placement="topRight" title={'GIS信息提示'}>
...
class modifyModal extends Component {
    ...
    hideModal(e) {   //点击确定的回调
        const {id, landName, area, typeName, longitude, latitude, user, gisData,baseId,stauts, userid} = this.props.fields;
            ...
            if (!err) {
                if(this.props.saveFlag) {
                  if (modaltype_land === 'add') {
                    ...
                    let userids;
                    typeof(user.value)==='string' ? userids=userid.value : user.value;
                    const modifydata = {
                      id: id.value,
                      name: landName.value,
                      modifyUserId: 1,
        ...
        return (
            <div>
                <Modal
                    ...
                    okText="确认"
                    cancelText="取消"
                    wrapClassName='farming-admin-modal farming-land-modal'
                >
...
const mapstateProps = (state) => {
    ...
    const {modalflag_land,modaltype_land,EditData,fields,allLandType, Rditdate} = state.baseReducer;
    return {

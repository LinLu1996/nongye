...
import {Input, message,Select,Button,Row,Col,Card,Form,Icon} from 'antd';
...
class Resources extends Component {
        ...
        if(record.type==='company') {
            if(e) {
                if(this.state.queryF) {
                    if(this.state.changedata.id===record.id) {
                        e.stopPropagation();
                    }
                }
            }
    ...
    async editor(record, e) {
        ...
        if(record.type==='base') {
            ...
            //await this.props.getOne({':id': Number(row.baseId)});
            this.props.defaultFields({
                functionaryList: {
                    value: this.state.personList
                },
            ...
            if(node.props.type==='company') {
                ...
                if(!selected) {
                    ...
                    this.setState({
                        ...
                        queryFlag: false,
                        Treeflag: false,
                        selectID:Number(node.props.id.split('|')[0]),
                        showAddBase:false,
                        showAddLand:true,
                        currentRecord:node.props.title.props.item
                    });
    ...
    renderAdvancedForm() {
        ...
        return (
            <Form layout="inline">
                <Row gutter={{md: 8, lg: 24, xl: 48}}>
                    <Col md={8} sm={24}>
                        <FormItem label="基地名称" labelCol="3">
                                <Input value={this.state.baseName}
                                       onChange={this.setBaseName.bind(this)}/>
                        </FormItem>
                    </Col>
                    <Col md={8} sm={24}>
                        <FormItem label="地块名称" labelCol="3">
                                <Input value={this.state.name} onChange={this.setName.bind(this)}/>
                        </FormItem>
                    </Col>
                    <Col md={8} sm={24}>
                        <FormItem label="负责人" labelCol="3">
                    ...
                    <div>
                        ...
                        <div className='tableList'>
                            ...
                            <Row style={{marginBottom: '20px'}}>
                                <Col span={24}>
                                    {
                                        this.state.showAddBase && addRole &&
                                        <Button style={{marginRight: '20px'}} type="primary" onClick={this.addmodelBase.bind(this,'company')}>新增基地</Button>
                                    }
                                    {
                                        this.state.showAddLand && addRoleLand &&
                                        <Button type="primary" onClick={this.addmodelBase.bind(this,'base')}>新增地块</Button>
                                    }
                                </Col>
                            </Row>
                            ...
                            <ModalForm names={this.state.name} setPosition={this.setPosition.bind(this)} updateTree={this.updateTree.bind(this)} userId={this.state.userId} props={this.props} checkName={this.checkName.bind(this)} queryRole={queryRole} saveFlag={this.state.saveFlag} selectID={this.state.selectID}/>
                            <ModalFormLand userId={this.state.userId} updateTree={this.updateTree.bind(this)} landNames={this.state.name} baseNames={this.state.address}props={this.props} checkName={this.checkLandName.bind(this)} queryRole={queryRole} saveFlag={this.state.saveFlag} selectID={this.state.selectID}/>

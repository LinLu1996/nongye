...
import {Modal, Input, Form, message, Select, Icon, Table, LocaleProvider, Popover} from 'antd';
...
const CustomizedForm = Form.create({
    ...
    return (
        <Form layout="inline">
            <FormItem label="品种明细">
            <div className='tishi'>{getFieldDecorator('name', {
                    rules: [{required: true, message: '请填写品种明细'}]
                })(<Input onChange={(e) => {
                    props.checkName(e.target.value, id,'checkName_crop');
                }}/>)}<Popover placement="left" content={content}><i className='iconfont icon-tishi1 crop_tishi'/></Popover>
                </div>
            </FormItem>
            ...
            <FormItem label="品种大类">
                {getFieldDecorator('categoryOne', {
                    rules: [{required: true, message: '请选择品种大类'}]
            ...
            <FormItem label="作物种类">
                {getFieldDecorator('categoryTwo', {
                    rules: [{required: true, message: '请选择作物种类'}]
                })(<Select style={{width: 300}} onChange={(e) => {
                    props.changeSmall(e);
                }}>

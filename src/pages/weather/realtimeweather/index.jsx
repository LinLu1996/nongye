...
import {Card, Button, Select, message, Row, Col, Form} from 'antd';
...
class WeatherOnline extends Component {
    ...
    reset () {
        this.setState({baseLand: null});
        this.query(0);
    }
    search () {
        const val = this.state.baseLand;
        this.query(val);
        this.queryHour(val);
        const s = [];
        for (let i = 0; i < this.props.dicList.length; i++) {
            s.push(this.props.dicList[i].windDirection);
        }
        this.setState({
            hourWindD: s
        });
            ...
            <div className='farming-box weather-box weather'>
                <Card bordered={false}>
                    <div className='content'>
                        <div className='time' style={{width: '100%',textAlign: 'right',marginBottom: '20px'}}>{this.state.now}</div>
...
const WrappedNormalLoginForm = Form.create()(WeatherOnline);

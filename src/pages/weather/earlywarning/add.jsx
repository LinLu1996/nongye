...
import {Select, Form, Input, Card, Button, message, Row, Col} from 'antd';
...
class Resources extends Component {
        ...
        return (
            <Form layout="inline">
                <Row gutter={{md: 8, lg: 24, xl: 48}}>
                    <Col md={8} sm={24}>
                        <FormItem label="基地">
                            <Select placeholder='请选择基地' onChange={this.setBaseId.bind(this)} className="weather-base">
                                {
                                    baseList.map((item) => {
                    ...
                    <Col md={16} sm={24} style={{textAlign: 'right'}}>
                        <div className='warning-title-r'>{this.state.dates}</div>
                    </Col>
                </Row>
            </Form>
        );
    ...
    render () {
        const {disasterGrade, temperature, humidity, windSpeed, title, cropId, disaster, period} = this.state;
        const {windSpeed_val,temperature_val,humidity_val} = this.state;
        const {periodArr, cropDicList} = this.props;
        return (
                ...
                <div className='content' style={{background: '#ffffff'}}>
                    {/*<div className='farming-search'>
                        <div className='farming-title title-other'>
                            ...
                            <div className='warning-title-r' style={{textAlign: 'right'}}>{dates}</div>
                        </div>
                    </div>*/}
                    ...
                    <div className='add-list' style={{background: '#ffffff'}}>
                        ...
                        <Row className='center-btn'>
                            <Button type="primary"  className='add-warning-btn' onClick={this.handleSubmit.bind(this)}>确定</Button>
                            <Button  style={{marginLeft:20}} className='add-warning-btn' onClick={this.handleReturn.bind(this)}>返回</Button>
                        </Row>
                    </div>
                </div>
                </Card>
